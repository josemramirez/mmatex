...
\usepackage[off]{auto-pst-pdf} % Added [off] to avoid shell-escape error; note: this may not render PSTricks properly. To fully render, use pdflatex with --shell-escape and remove [off].
\usepackage{tikz}
\usepackage{hyperref}
\usepackage{epstopdf} % For handling .eps files; requires --shell-escape.

\hypersetup{
pdfpagemode={FullScreen},
pdftitle={MRU h1: ver1},
pdfauthor={J.M.~Ram{'i}rez},
pdfcreator={J.M.~Ram{'i}rez},
pdfproducer={J.M.~Ram{'i}rez},
pdfkeywords={Movimiento,Rectilineo,Uniforme}}

\def\psPi{3.14159265}

% Assuming lib/FORMULAS and lib/NOTASUNIDADES are available; if not, define needed macros here.
% For example, if \vNUa is a note, define it as empty or remove its call.
% \def\vNUa{} % Dummy definition if needed.
...
\vNUa % Commented out to avoid undefined if file not present; restore if defined.
\end{frame}
...
\frac{\text{Se recorren 10 km ({\color{red} distancia})}}
{\text{en 1 hora ({\color{red} tiempo})}}
\end{equation*}

Las otras dos f\'ormulas que se derivan de (\ref{vmru}) son:

{\large \( x = vt \) \\ \( t = \frac{x}{v} \)}
\end{frame}
...
\rput(0.87,-1.44){\includegraphics[width=1cm]{images/ciclista1.pdf}}
\rput(11.39,2.2){\includegraphics[width=1cm]{images/ciclista1.pdf}}
...
\rput(0.97,4.64){\includegraphics[width=3cm]{images/policia_transito.pdf}}
...
Entonces:
	...
	{\huge
	\( x_0 = v_1t_{0}= \)
	\( (50~{\color{red} [\frac{1000}{3600}]}~\frac{\text{m}}{\text{s}})
        (5~\text{s}) \)
